...
def deserialize_header(data):
    ...
    if msg['magic_number'] != MAGIC_NUMBER:
        raise ValueError("{} != {}".format(hexlify(msg['magic_number']), hexlify(MAGIC_NUMBER)))
...
def deserialize_msg(data):
    ...
    if (data_len - HEADER_LEN) < msg['length']:
        ...
        raise PayloadTooShortError(
            "got {} of {} bytes".format(data_len, HEADER_LEN + msg['length']))
...
class Connection(object):
    def __init__(self, to_addr, socket_timeout=10, from_addr=("0.0.0.0", 0), **conf):
    ...
    def open(self):
        self.socket = create_connection(
            self.to_addr, timeout=self.socket_timeout, source_address=self.from_addr,
            proxy=self.proxy)
...
def test_connection(to_addr):
    to_services = 1  # NODE_NETWORK
    conn = Connection(to_addr, to_services=to_services, socket_timeout=5)
...
if __name__ == '__main__':
    from v8.engine.handlers.node_handler import get_all_not_valid_node
    from v8.config import config, config_online
    config.from_object(config_online)

    all_not_valid_node = get_all_not_valid_node()
    for _node in all_not_valid_node:
        to_addr = (_node['ip'].split(':')[0], int(_node['ip'].split(':')[1]))
        try:
            test_connection(to_addr)
