...
def test_get():
    ...
    while True:
        try:
            ...
            d = ret['data']
            price = d['lbtcusdt']['price']
            if abs(price - price_init) / price_init > 0.09:
                t = time.localtime()
                if (6 > t.tm_hour or t.tm_hour > 22) and abs(price - price_init) / price_init < 0.12: continue
                price_init = price
                print(ret['serverTime'], 'lbtc', price)
                if first:
                    first = False
                else:
    ...
    for k in ['bchusdt', 'bsvusdt', 'lbtcusdt', 'ethusdt', 'btcusdt']:
